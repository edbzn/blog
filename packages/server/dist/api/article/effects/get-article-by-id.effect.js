...
exports.getArticleByIdEffect$ = req$ => req$.pipe(core_1.use(validator$), operators_1.mergeMap(req => rxjs_1.of(req.params.id).pipe(operators_1.mergeMap(article_dao_1.ArticleDao.findById), operators_1.mergeMap(never_nullable_1.neverNullable), operators_1.map(article => ({ body: article })), operators_1.catchError(() => rxjs_1.throwError(new core_1.HttpError('Article does not exist', core_1.HttpStatus.NOT_FOUND))))));
